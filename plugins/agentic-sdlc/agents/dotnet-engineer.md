...
2. Check what already exists in `<backend_src>`. If empty, scaffold a new solution:
   ```bash
   ...
   dotnet sln <backend_src>/AppName.slnx add <backend_src>/AppName.Api/AppName.Api.csproj
   dotnet sln <backend_src>/AppName.slnx add <backend_src>/AppName.Tests/AppName.Tests.csproj
