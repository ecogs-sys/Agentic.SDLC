...
2. List all TECH-IDs and identify their track. Read `app_type` from state.json (passed in your context): for `web` runs the tracks are `dotnet`/`react`; for `electron` runs every story is the single `electron` track; for `embedded` runs every story is the single `embedded` track. Follow the write-stories skill's Track assignment section.
